...
Typing word name followed by return prints this word's definition.

Typing word name followed by other words inserts them to a definition. By default words
are inserted at the beginning. Word starting with @/@ changes place of insertion to
after next such word. Multiple @/@ select Nth next word.

Slash after word wipe it and more slashes wipe words after it. Inserting will be done after wiped words.
Only slash sets insertion to the end of word.
...
Changes are not commited to running code until lone @!@ followed by return is entered.

Lone @?@ shows current changed words.
